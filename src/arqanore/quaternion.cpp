...
arqanore::Vector3 arqanore::Quaternion::euler_angles(arqanore::Quaternion q) {
    auto glm_quat = glm::quat(q.w, q.x, q.y, q.z);
    auto glm_vec = glm::degrees(glm::eulerAngles(glm_quat));

    return Vector3(glm_vec.x, glm_vec.y, glm_vec.z);
}

arqanore::Quaternion arqanore::Quaternion::normalize(arqanore::Quaternion q) {
    auto glm_quat = glm::quat(q.w, q.x, q.y, q.z);
...
arqanore::Quaternion arqanore::Quaternion::rotate(arqanore::Quaternion q, float angle, arqanore::Vector3 axis) {
    auto glm_quat = glm::quat(q.w, q.x, q.y, q.z);
...
arqanore::Quaternion arqanore::Quaternion::rotate(arqanore::Quaternion q, arqanore::Vector3 euler) {

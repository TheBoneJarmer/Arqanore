...
namespace Seanuts.Logic.PathFinding
{
    ...
    {
        public static List<SNPosition> FindPath(SNGrid grid, SNPosition start, SNPosition end, SNOffset[] movementPattern)
        {            
            ClearStepList();

            if (start == end)
            {
                return new List<SNPosition> {start};
            }
           
            var head = new SNMinHeapNode(start, ManhattanDistance(start, end));
            var open = new SNMinHeap();            
            open.Push(head);

            var costSoFar = new float[grid.DimX * grid.DimY];
            var cameFrom = new SNPosition[grid.DimX * grid.DimY];                       
        ...
        public static List<SNPosition> FindPath(SNGrid grid, SNPosition start, SNPosition end, SNOffset[] movementPattern, int iterationLimit)
        {
            ClearStepList();
            ...
            SNPosition end)
        {
            ...
            foreach (var option in GetMovementOptions(current, grid.DimX, grid.DimY, movementPattern))
            {
                ...
                open.Push(new SNMinHeapNode(position, expectedCost));

                MessageOpen(position);
            }
        }

        private static List<SNPosition> ReconstructPath(SNGrid grid, SNPosition start, SNPosition end, SNPosition[] cameFrom)
        {
        ...
        private static IEnumerable<SNOffset> GetMovementOptions(
            SNPosition position,
            int dimX,
            int dimY,
            IEnumerable<SNOffset> movementPattern)
        {

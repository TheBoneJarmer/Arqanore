...
namespace Seanuts.Logic.PathFinding
{
    ...
    {       
        ...
        public SNGrid(int dimX, int dimY, float defaultCost = 1.0f)
        {
            if (defaultCost < 1)
            {
        ...
        public void SetCellCost(SNPosition position, float cost)
        {
            if (cost < 1)
            {
                throw new ArgumentOutOfRangeException(
        ...
        /// </summary>
        /// <param name="position">A position inside the grid</param>
        public void BlockCell(SNPosition position) => SetCellCost(position, float.PositiveInfinity);

        /// <summary>
        /// Makes the cell traversable, gives it the default traversal cost as provided in the constructor
        /// </summary>
        /// <param name="position">A position inside the grid</param>
        public void UnblockCell(SNPosition position) => SetCellCost(position, this.DefaultCost);
        ...
        public float GetCellCost(SNPosition position)
        {
        ...
        /// <param name="position">A position inside the grid</param>
        /// <returns>The cost</returns>
        internal float GetCellCostUnchecked(SNPosition position)
        {
        ...
        public SNPosition[] GetPath(SNPosition start, SNPosition end)
            => GetPath(start, end, SNMovementPatterns.Full);
        ...
        /// <param name="start">The start position</param>
        /// <param name="end">The end position</param>
        /// <param name="movementPattern">The movement pattern of the agent, <see cref="SNMovementPatterns"/> for several built-in options</param>
        /// <returns>Positions along the shortest path from start to end, or an empty array if no path could be found</returns>
        public SNPosition[] GetPath(SNPosition start, SNPosition end, SNOffset[] movementPattern)
        {
            var current = SNPathFinder.FindPath(this, start, end, movementPattern);

            if (current == null)
            {
                return new SNPosition[0];
            }
            ...
            var steps = new Stack<SNPosition>();
        ...
        /// <returns>Positions along the shortest path from start to end, or an empty array if no path could be found</returns>
        public SNPosition[] GetPath(SNPosition start, SNPosition end, SNOffset[] movementPattern, int iterationLimit)
        {
            var current = SNPathFinder.FindPath(this, start, end, movementPattern, iterationLimit);

...
{
    public class SNAsyncHttpServer
    {
        ...
        public SNAsyncHttpServer() : this(80)
        {

        }
        public SNAsyncHttpServer(int port)
        {
            this.port = port;
        ...
        private void ReceiveCallback(IAsyncResult result)
        {
            try
            {
                ...
                if (bytesRead > 0)
                {
                    ...
                    {
                        SNHttpServerRequest request = null;
                        SNHttpServerResponse response = null;
                        SNHttpConnectionInfo info = null;

                        try
                        {
                            info = new SNHttpConnectionInfo(socketMessage.Socket);
                            response = new SNHttpServerResponse(socketMessage.Socket);
                        ...
                        try
                        {
                            ...
                            if (MaxUploadSize == 0)
                            {
                                request = new SNHttpServerRequest(socketMessage.Data);
                            }
                            else
                            {
                                request = new SNHttpServerRequest(socketMessage.Data, MaxUploadSize);
                            }
        ...
        /* EVENTS */
        public delegate void OnRequestHandler(SNHttpServerRequest request, SNHttpServerResponse response, SNHttpConnectionInfo info);

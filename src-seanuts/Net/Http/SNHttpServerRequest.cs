...
{
    public class SNHttpServerRequest
    {
        ...
        public SNHttpRequestBody Body { get; set; }
        public string Path { get; set; }
        public string Raw { get; set; }
        
        public SNHttpServerRequest()
        {
            this.Headers = new SNHttpHeaders();
            this.Cookies = new Dictionary<string, string>();
            this.Files = new List<SNHttpRequestFile>();
            this.Query = new Dictionary<string, string>();
        }

        public SNHttpServerRequest(byte[] data) : this()
        {
            Raw = Encoding.ASCII.GetString(data);
            this.Parse(data, 2 * 1024 * 1024);
        }
        public SNHttpServerRequest(byte[] data, int maxContentLength) : this()
        {
            Raw = Encoding.ASCII.GetString(data);
        ...
        private void ParseFirst(string line)
        {
            ...
            // Set the path
            this.Path = SNHttpHelper.ConvertHexCodes(path);
            ...
            if (this.Method == HttpMethod.Get && path.Contains("?"))
            {
                ...
                foreach (string keyvalue in queryString.Split('&'))
                {
                    ...
                    Query.Add(key, SNHttpHelper.ConvertHexCodes(value));
                }
            }

            // Set the version
            if (version == "HTTP/1.1") this.Version = SNHttpVersion.Http11;
        }
        ...
        private void ParseBody(List<byte> data)
        {
            ...
            {
                Body = new SNHttpRequestBody(Encoding.ASCII.GetString(data.ToArray()), contentType[0]);
            }
        ...
        {
            var current = new SNHttpRequestFile();
            var newLine = false;
            var carriageReturn = false;
            var originalLine = "";
            var line = "";
            ...
            // Set the body
            this.Body = new SNHttpRequestBody(keyValueData, "multipart/form-data");
        }
        ...
        private void ValidateContentType()
        {
            ...
            if (contentType.Length > 0)
            {
                ...
                if (!valid)
                {
                    throw new SNHttpException(HttpStatusCode.UnsupportedMediaType, $"Content type {contentType[0]} not supported");
                }
            }
            else
            {
                throw new SNHttpException(HttpStatusCode.UnsupportedMediaType, $"Header 'content-type' missing");
            }
        }
        private void ValidateContentLength(int maxLength)
        {
            ...
            {
                throw new SNHttpException(HttpStatusCode.LengthRequired, $"Header 'content-length' missing");
            }

            // Check the content length
            if (Headers.ContentLength > maxLength)
            {
                throw new SNHttpException(HttpStatusCode.BadRequest, "Maximum content length exceeded");
            }
        }
        private void ValidateMethod()
        {
            if (Method == null)
            {
                throw new SNHttpException(HttpStatusCode.MethodNotAllowed, "The request method is not allowed");
            }
        }
    }
}
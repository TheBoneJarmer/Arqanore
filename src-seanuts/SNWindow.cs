...
namespace Seanuts
{
    public class SNWindow
    {
        ...
        public SNColor ClearColor
        {
            get { return clearColor; }
            set { clearColor = value; }
        }

        public SNWindow(int width, int height, string title)
        {
            // Set the properties
            this.Width = width;
            this.Height = height;
            this.Title = title;
            this.ClearColor = SNColor.BLACK;
        }
        ...
        {
            Console.WriteLine($"GL Version: {SNDevice.GLVersion}");
            Console.WriteLine($"GLSL Version: {SNDevice.GLSLVersion}");
            Console.WriteLine();
        }
        ...
        private void Sync(bool pollEvents)
        {
            ...
            // Update time
            SNTime.Now = GLFW.glfwGetTime();

            // Main loop
            while (GLFW.glfwWindowShouldClose(Handle) == 0)
            {
                // Update time
                SNTime.Then = SNTime.Now;
                SNTime.Now = GLFW.glfwGetTime();
